...
public class FileManager
{
  ...
  public FileManager(Stream stream, String filename)
  {
    this.stream = stream;
  ...
  public List<Servico> Relatorio()
  {
    ...
    var servicos = new List<Servico>();
    {
      while (!streamsanitizado.EndOfStream)
      {
        ...
        servico.id_servico_situacao = values[3];
        ...
        if(servico.codigos_concaternados == "20.5") servico.id_servico_situacao = "concluído";
        servico.Valoracao();
  ...
  public List<Composicao> Composicao()
  {
    ...
    using (var reader = new ExcelPackage(stream))
    {
      ...
      for(int row = 2; row <= rowCount; row++)
      {
        ...
        composicao.atividade = worksheet.GetValue<string>(row, cabecalho["atividade"]);
        composicao.SanitizarAlavanca();
        composicao.IsPesado();
        composicao.IsNoturno();
        ...
        composicao.supervisor = worksheet.GetValue<string>(row, cabecalho["supervisor"]);

        if (cabecalho.TryGetValue("id_supervisor", out Int32 coluna))
        {
          temp = worksheet.GetValue<string>(row, coluna);
          composicao.id_supervisor = Conversor.GetNumberMiddle(temp);
          if(composicao.id_supervisor == null)
            composicao.validacao.Add($"A matrícula sup. informada ({temp}) é inválida!");
        }

        composicao.regional = worksheet.GetValue<string>(row, cabecalho["regional"]);
        composicao.SanitizarRegional();

        composicao.controlador = cabecalho.TryGetValue("controlador", out coluna) ? worksheet.GetValue<string>(row, coluna) : null;
